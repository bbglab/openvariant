.. _Examples:

*********************
Examples
*********************

.. toctree::
   :maxdepth: 4
   :hidden:

   examples/find_files_examples.rst
   examples/variant_examples.rst
   examples/tasks_examples.rst
   examples/cli_examples.rst
   examples/plugin_examples.rst

.. panels::
    :container: container-lg pb-2
    :body: text-justify
    :header: text-justify font-weight-bold

    :column: col-lg-12 border-0 p-1


    .. link-button:: examples/find_files_examples
        :type: ref
        :text: 🔍  Find files
        :classes: stretched-link text-justify p-0 m-0

    +++

    Find input files with its corresponding annotation file.

    ---
    :column: col-lg-12 border-0 p-1

    .. link-button:: examples/variant_examples
        :type: ref
        :text: 📄  Variant
        :classes: stretched-link text-justify p-0 m-0

    +++

    Parse input files.

    ---
    :column: col-lg-12 border-0 p-1

    .. link-button:: examples/tasks_examples
        :type: ref
        :text: 📝  🧮  🗃️  Tasks
        :classes: stretched-link text-justify p-0 m-0

    +++

    Bunch of tasks to perform with **OpenVariant**

    - Cat
    - Count
    - Group by

    ---
    :column: col-lg-12 border-0 p-1

    .. link-button:: examples/cli_examples
        :type: ref
        :text: 🖥️  Command-line (CLI)
        :classes: stretched-link text-justify p-0 m-0

    +++

    Bunch of commands to run different tasks.

    ---
    :column: col-lg-12 border-0 p-1

    .. link-button:: examples/plugin_examples
        :type: ref
        :text: 🧩 Plugin
        :classes: stretched-link text-justify p-0 m-0

    +++

    Different functionalities that plugin system contains.

