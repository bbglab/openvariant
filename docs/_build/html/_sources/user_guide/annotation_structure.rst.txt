...
way that previously has been defined. `Annotation` will describe how the `output` will be and how `input` files will be parsed.
...
to search and match the files that ends with that pattern.
...
List of columns that we want as an `output`. All value must be values of the annotation fields. If it is
empty the default value will be all the fields described on the `Annotation <#annotation>`_ parameter.
...
List of the different annotations that `output` file will have. The list of each field
...
Transformation of `input` values to the `output`. It will match the ``fieldSource`` that appear on the `input` file and
...
It will get the name of the `input` file that is processing on that moment
as value of the ``field``. Also, the filename will be executed through ``function`` parameter and ``regex`` parameter as an input.
...
Allows to use `mapping` file to apply a new value based on a many-to-one relation between `input` file and the
`mapping` file. This annotation have different parameter that we can describe as:
...
It will take ``fieldSource`` on the `input` file and it will match with the value of ``fieldMapping`` on `mapping` file.
When this two values match it will get the value of `fieldValue` on that row as value of the output ``field``.
...
List of values that will be exclude from the `output` file in the parsing process. This parameter is optional and all the elements
will be treated as an `OR` operation, it means if any of the elements is turns true the row will be excluded.
...
Each element is composed of a ``field`` parameter and a ``value`` parameter. If ``value`` matches with the same ``field``
that appears on the `output`, the row that is reading on that moment will be excluded. Both parameters are required.
