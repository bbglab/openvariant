...
class TestCount(unittest.TestCase):
    ...
    # Count functionality
    def test_count(self):
        res = count(f'{os.getcwd()}/tests/data/', f'{os.getcwd()}/tests/data/task_test.yaml', quite=True)
        self.assertEqual(res[0], 42130, "Count number not matching")
        self.assertEqual(res[1], {}, "Count groups not matching")

    def test_invalid_count(self):
        with self.assertRaises(FileNotFoundError):
            res = count(f'{os.getcwd()}/tests/data/no-exist', f'{os.getcwd()}/tests/data/no-exist.yaml', quite=True)

    # Using where attribute
    def test_count_equal_where(self):
        res = count(f'{os.getcwd()}/tests/data/', f'{os.getcwd()}/tests/data/task_test.yaml', where="DATASET == \"acc\"", quite=True)
    ...
    def test_count_little_than_where(self):
        res = count(f'{os.getcwd()}/tests/data/', f'{os.getcwd()}/tests/data/task_test.yaml', where="A < 52366244", quite=True)
    ...
    def test_count_no_exist_where(self):
        res = count(f'{os.getcwd()}/tests/data/', f'{os.getcwd()}/tests/data/task_test.yaml',
    ...
    def test_count_bad_syntax_where(self):
        with self.assertRaises(ValueError):
            count(f'{os.getcwd()}/tests/data/', f'{os.getcwd()}/tests/data/task_test.yaml',
                  where="NO_EXIST = \"no_exist\"", quite=True)

    # Using group by attribute
    def test_count_group_by(self):
        res = count(f'{os.getcwd()}/tests/data/', f'{os.getcwd()}/tests/data/task_test.yaml', group_by="DATASET", quite=True)
        self.assertEqual(res[0], 42130, "Count number not matching")
    ...
    def test_count_invalid_group_by(self):
        res = count(f'{os.getcwd()}/tests/data/', f'{os.getcwd()}/tests/data/task_test.yaml', group_by="NO_EXIST", quite=True)
        self.assertEqual(res[0], 0, "Count number not matching")
        self.assertEqual(res[1], {}, "Count groups not matching")
